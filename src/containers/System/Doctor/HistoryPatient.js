...
class ManageHistory extends Component {
    ...
    getFilteredData = () => {
        ...
        return dataHistory.filter(item => {
            ...
            return patientName.includes(query) || email.includes(query) || address.includes(query) || description.includes(query);
        });
    }
    ...
    render() {
        const { dataHistory, isModalOpen, selectedPatient, currentPage, itemsPerPage } = this.state;
